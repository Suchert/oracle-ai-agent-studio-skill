...
description: >
  Guide for designing, building, deploying, testing, and managing AI agents in Oracle AI Agent Studio
  for Oracle Fusion Cloud Applications. Use this skill whenever the user mentions Oracle AI Agent Studio,
  Oracle Fusion AI agents, Fusion agent teams, agent designer, agent templates, Oracle AI Agent Marketplace,
  OCI Generative AI Agents, Oracle agent workflows, or asks about building enterprise AI agents on Oracle Cloud.
  Also trigger for Oracle HCM/ERP/SCM/CX agent customization, agent evaluation and observability in Fusion,
  multi-agent orchestration in Oracle, MCP integration with Fusion agents, deterministic agent workflows,
  human-in-the-loop patterns in Oracle, agent tracing and token monitoring, or connecting Fusion agents to
  third-party LLMs (OpenAI, Anthropic, Cohere, Google, Meta, xAI). Trigger even if user says "Oracle agents",
  "Fusion AI", "agent studio", or references specific pre-built agents like Payables Agent, Career Coach,
  Expense Policy Agent, or any Oracle Fusion AI capability.
---
